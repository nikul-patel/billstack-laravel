...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('role_has_permissions', function (Blueprint $table) use ($teams, $teamForeignKey) {
            $table->unsignedBigInteger('permission_id');
            $table->unsignedBigInteger('role_id');

            if ($teams || config('permission.testing')) {
                $table->unsignedBigInteger($teamForeignKey)->nullable();
                $table->index($teamForeignKey, 'role_has_permissions_team_foreign_key_index');
            }
            ...
            $table->primary(
                $teams || config('permission.testing')
                    ? [$teamForeignKey, 'permission_id', 'role_id']
